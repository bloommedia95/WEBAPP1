...
const Cosmetic = () => {
	...
	// Get filter config based on category (default to cosmetic)
	const getCategoryKey = (categoryName) => {
		if (!categoryName) return 'cosmetic';
		const key = categoryName.toLowerCase();
		return filterConfig[key] ? key : 'cosmetic';
	};

	const categoryKey = getCategoryKey(catName);
	const filterData = filterConfig[categoryKey] || filterConfig.cosmetic;

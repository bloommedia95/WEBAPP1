...
const Footwear = () => {
	...
	// Get filter config based on category (default to footwear)
	const getCategoryKey = (categoryName) => {
		if (!categoryName) return 'footwear';
		const key = categoryName.toLowerCase();
		return filterConfig[key] ? key : 'footwear';
	};

	const categoryKey = getCategoryKey(catName);
	const filterData = filterConfig[categoryKey] || filterConfig.footwear;

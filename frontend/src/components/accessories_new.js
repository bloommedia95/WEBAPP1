...
const Accessories = () => {
	...
	// Get filter config based on category (default to accessories)
	const getCategoryKey = (categoryName) => {
		if (!categoryName) return 'acessiories'; // Note: typo in filterConfig
		const key = categoryName.toLowerCase();
		// Handle the typo in filterConfig
		if (key === 'accessories') return 'acessiories';
		return filterConfig[key] ? key : 'acessiories';
	};

	const categoryKey = getCategoryKey(catName);
	const filterData = filterConfig[categoryKey] || filterConfig.acessiories;
